...
public:
  ...
  [[nodiscard]]
  std::future<typename MethodType::ReturnType> Call(const RPCTarget& target,
                                                    Args&&... args);
  ...
  RpcRequestMessage request{.methodId = MethodType::Id,
                            ...
                            .payload =
                                std::vector<uint8_t>(writeArgs.bytes().begin(),
                                                     writeArgs.bytes().end())};
  MessageSystem::Get().SendMessage(request, target,
                                   MessageSendMode::eReliableBatched);
  return std::make_pair(request.methodId, request.callID);
}

template <typename MethodType>
inline void AtlasNet::RPC::SendResponse(const RPCTarget& target,
                                        RPC_Internal::CallID callID,
                                        const MethodType::Return& ret)
{
  ByteWriter writeArgs;
  writeArgs(ret);
  RpcResponseMessage request{.methodId = MethodType::Id,
                             .callID = GetNextCallID(MethodType::Id),
                             .payload =
                                 std::vector<uint8_t>(writeArgs.bytes().begin(),
                                                      writeArgs.bytes().end())};
