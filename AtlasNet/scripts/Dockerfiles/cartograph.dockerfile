...
RUN --mount=type=cache,target=/atlasnet/build/vcpkg_installed cmake -S . -B /atlasnet/build -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ \
...
COPY --from=atlasnetsdk runtime/cartograph/web/. ./
RUN  npm install
COPY --from=builder /atlasnet/runtime/cartograph/web/nextjs ./nextjs/
#COPY --from=builder /atlasnet/runtime/cartograph/web ./
