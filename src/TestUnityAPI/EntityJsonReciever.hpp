...
inline void PrintEntityFromJson(const std::string& body) {
    ...
    try {
        ...
        std::string id;
        if (!j.contains("id")) { std::cout << "[KDNet] missing 'id'\n"; return; }
        ...
        if (!p.is_array() || p.size()!=3) { std::cout << "[KDNet] position must be [x,y,z]\n"; return; }
        float px = to_f(p[0]), py = to_f(p[1]), pz = to_f(p[2]);

        // --- print result ---
        std::cout << "[KDNet] id=" << id
                  << " pos=(" << px << "," << py << "," << pz << ")"
                  << std::endl; // flush
    }
    catch (const std::exception& e) {
        std::cout << "[KDNet] parse error: " << e.what() << std::endl;
    }
}
